...
// Send WhatsApp reminders to the owner for tasks that are due and not yet
// reminded. Uses broadcast_settings.owner_phone. Returns how many were sent.
export async function processTaskReminders(supabase) {
  const { data: settings } = await supabase.from('broadcast_settings').select('owner_phone').eq('id', 1).single();
  if (!settings?.owner_phone) return 0;
  ...
  let sent = 0;
  for (const t of due || []) {
    const when = new Date(t.due_at).toLocaleString('he-IL', { timeZone: 'Asia/Jerusalem', dateStyle: 'short', timeStyle: 'short' });
    const res = await sendWhatsApp(settings.owner_phone, `⏰ תזכורת: ${t.title}${t.details ? `\n${t.details}` : ''}\n(${when})`);
    if (!res.ok) continue;
    await supabase.from('admin_tasks').update({ reminded_at: new Date().toISOString() }).eq('id', t.id);
    sent++;
  }
  return sent;
}
