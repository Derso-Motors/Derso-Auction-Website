...
export async function postToSocialync(imageUrl, caption, platforms) {
...
export async function scheduleToSocialync(imageUrl, caption, scheduledFor, platforms) {
  const session = await mcpSession();
  const plats = platforms || DEFAULT_PLATFORMS;

  await mcpCall(session, 'validate_post', {
    platforms: plats,
    caption,
    imageUrls: [imageUrl],
    scheduledFor,
  ...
  return mcpCall(session, 'publish_scheduled', {
    platforms: plats,
    caption,
    imageUrls: [imageUrl],
    scheduledFor,
...
export async function postToAll(imageUrl, caption) {
  return postToSocialync(imageUrl, caption);
}
