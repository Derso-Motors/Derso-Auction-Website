...
import { guidelinesForPrompt } from './social-guidelines';
...
export async function generateCaption(kind, payload, supabase) {
  ...
  const brief = {
    ...
    topic: `נושא חם מהניטור שלנו: ${JSON.stringify(payload)}. בנה מזה פוסט שירותי — פתח בנקודת הכאב, תן את העובדות והמספרים, וסגור במה הצופה מרוויח.`,
  }[kind] || JSON.stringify(payload);
  return aiChat([
    {
      role: 'system',
      content: 'אתה כותב תוכן שיווקי לעמודי הסושיאל של "דרסו רכבים" — בית ליווי מקצועי לעולם הנהיגה והרכב בישראל.\n' +
        guidelinesForPrompt() +
...
export async function publishPost(supabase, post) {
  ...
  try {
    // Carousel when we have slides, single image otherwise
    const result = await postToAll(post.image_urls?.length ? post.image_urls : post.image_url, post.caption);
