...
import { postToFacebook, postToInstagram, metaConfigured } from './meta';
...
export async function publishPost(supabase, post) {
  if (!metaConfigured()) throw new Error('Meta tokens not configured (META_PAGE_ID / META_PAGE_ACCESS_TOKEN)');
  try {
    const fbId = await postToFacebook(post.image_url, post.caption);
    let igId = null;
    try { igId = await postToInstagram(post.image_url, post.caption); } catch (e) { console.error('IG publish failed:', e.message); }
    await supabase.from('social_posts').update({ status: 'posted', fb_post_id: fbId, ig_post_id: igId, posted_at: new Date().toISOString(), error: null }).eq('id', post.id);
    return { ok: true, fbId, igId };
  } catch (e) {
