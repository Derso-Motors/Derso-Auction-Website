...
// Self-healing gcal sync: any future meeting without a gcal_event_id gets
// one created (covers meetings created before sync existed, or when the
// Apps Script call failed at creation time). Runs on the 10-minute cron.
export async function syncMeetingsToGcal(admin) {
  const url = process.env.APPS_SCRIPT_URL;
  if (!url) return { synced: 0, skipped: 'no APPS_SCRIPT_URL' };
  const out = { synced: 0, errors: [] };
  const { data: meetings } = await admin.from('meetings')
    ...
    .limit(20);
  for (const m of meetings || []) {
    const who = m.profiles?.full_name || m.client_name || '';
    const phone = m.profiles?.phone || m.client_phone || '';
    try {
      const res = await fetch(url, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'createMeeting',
          title: `${m.title}${who ? ` — ${who}` : ''}`,
          startISO: new Date(m.scheduled_at).toISOString(),
          durationMin: 15,
          notes: [m.location || '', phone ? `טלפון: ${phone}` : '', m.notes || ''].filter(Boolean).join('\n'),
        }),
      });
      const j = await res.json().catch(() => null);
      if (j?.eventId) {
        await admin.from('meetings').update({ gcal_event_id: j.eventId }).eq('id', m.id);
        out.synced++;
      } else {
        out.errors.push(`gcal ${m.id}: no eventId`);
      }
    } catch (e) {
      out.errors.push(`gcal ${m.id}: ${e?.message || 'fetch failed'}`);
    }
  }
  return out;
}
