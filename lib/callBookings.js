...
// Characterization calls: Sun-Thu, 12:30-14:15 Israel time, 15 minutes each (last slot 14:00).
export const SLOT_TIMES = [];
for (let h = 12, m = 30; h < 14 || (h === 14 && m < 15); ) {
  SLOT_TIMES.push(`${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`);
  m += 15;
  if (m === 60) { m = 0; h += 1; }
}

// Office lunch break — always blocked for client bookings (13:15 up to 13:45).
// Slots after 14:00 no longer exist, so the old 13:15–14:45 window shrinks.
export function isLunchSlot(timeStr) {
